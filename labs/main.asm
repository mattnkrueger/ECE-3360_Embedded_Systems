...
toggle_fan:
	...
	com fan_state           ; Toggle the fan state
    tst fan_state           ; Test if fan_state is zero after complementing
    breq fan_is_off         ; If zero, fan should be off
    ; Fan should be ON
    mov dc_ocr2b, prev_dc    ; Restore previous duty cycle
    rjmp update_pwm
	fan_is_off:
		mov prev_dc, dc_ocr2b    ; Save current duty cycle before turning off
		ldi dc_ocr2b, 0          ; Set duty cycle to 0 (off)
    update_pwm:
		sts OCR2B, dc_ocr2b      ; Update PWM register
		sbi PINB, 4;
		pop r0
		out SREG, r0
		pop r0
